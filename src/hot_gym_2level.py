...
column_field2 = ColumnField(
    input_fields=[column_field],
    non_spatial=True,
    num_columns=config["num_columns"]*config["cells_per_column"],
    cells_per_column=config["cells_per_column"],
)
column_field.add_input_fields([column_field2])
...
for idx in tqdm(range(500)):
    ...
    column_field2.compute(learn=False)
