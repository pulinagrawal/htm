...
class Segment(Active, Learning):
    ...
    def __init__(
        ...
        self.max_synapses = int(.02*len(self.parent_cell.distal_field.cells))
        self.activation_threshold: float = .5
...
class ColumnField(Field):
    ...
    def learn_cells(self) -> None:
        ...
        for column in self.columns:
            for cell in column.cells:
              if cell.predictive and not cell.active:
                  for segment in cell.segments:
                      if segment.prev_active:
                          segment.weaken()  # Weaken synapses for unfulfilled predictions
